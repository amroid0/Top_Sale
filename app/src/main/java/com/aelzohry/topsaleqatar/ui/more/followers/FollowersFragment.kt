...
class FollowersFragment : BaseActivity<FragmentFollowersBinding, BaseViewModel>() {
    ...
    companion object {
        fun newInstance(mActivity: Context, userId: String?): Intent {
            val intent = Intent(mActivity, FollowersFragment::class.java)
...
//                }
    ...
    private fun loadData(page: Int, userId: String) {
        ...
        startLoading()
        followersCall = repository.getFollowers(page, userId) { users, messsage ->
            users?.let {
                if (it.isEmpty()) {
    ...
    private fun loadNextPage() {
        loadData(currentPage + 1, userId.toString())
    }
